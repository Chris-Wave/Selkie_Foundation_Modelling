...
def sliding_resistance(c, loads, eccent_cache, weight, phi, slope, Hs, SF):
    ...
    Hu_drained = (loads['Vv'] - (eccent_cache['Calc'].Wb + eccent_cache['Calc'].A
                             * Hs * weight) * math.cos(slope)) * \
                             math.tan(phi)
    Hu_undrained = eccent_cache['Calc'].A * c +2 *Hs * eccent_cache['Calc'].L*c
    sliding_force = (loads['Vv'] - eccent_cache['Calc'].Wb - Hs * weight *
                     eccent_cache['Calc'].A) * math.sin(slope) + (
                         loads['Vh'] * math.cos(slope))
    Hu_drained_SF = np.abs(Hu_drained/SF)
    Hu_undrained_SF = Hu_undrained / SF
    
    return (np.min([Hu_drained_SF, Hu_undrained_SF], axis = 0) > sliding_force)
